...
import math
import numpy as np
from ..profiles.profile_db import profile_mapper, MATERIAL_CATALOG, SECTION_GEOM
...
def ml_select_material(member: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ML Model Output:
    - material_index: 0=ASTM A36, 1=ASTM A572 Gr50, 2=ASTM A992, 3=ASTM A913 Gr65, 4=ASTM A572 Gr65, 5=ASTM A709 Gr70W, 6=Q460, 7=ASTM A852 Gr70
    - confidence: Model confidence (0-1)
    ...
    try:
        ...
        if classifier:
            ...
            # Map index to material name (updated for high-strength materials)
            material_names = ["ASTM A36", "ASTM A572 Gr50", "ASTM A992", "ASTM A913 Gr65", "ASTM A572 Gr65",
                             "ASTM A709 Gr70W", "Q460", "ASTM A852 Gr70"]
            if 0 <= material_idx < len(material_names):
                mat_name = material_names[material_idx]
            else:
                mat_name = 'ASTM A992'  # fallback to high-strength standard
                confidence = 0.5
            ...
            if role == 'column':
                ...
                if span_m > 12:
                    mat_name = 'Q460'  # High-strength for long spans
                    confidence = 0.85
                elif span_m > 6:
                    mat_name = 'ASTM A913 Gr65'  # ASTM high-strength
                    confidence = 0.80
                else:
                    mat_name = 'ASTM A572 Gr65'  # ASTM standard strength
                    confidence = 0.75
            else:  # brace
                if span_m > 15:
                    mat_name = 'Q460'  # High-strength for long braces
                    confidence = 0.85
                elif span_m > 8:
                    mat_name = 'ASTM A913 Gr65'  # ASTM high-strength
                    confidence = 0.80
                else:
                    mat_name = 'ASTM A572 Gr50'  # ASTM standard strength
                    confidence = 0.75
        ...
        if mat_name in MATERIAL_CATALOG:
            ...
            # Ultimate fallback - use ASTM A572 Gr50 as default
            material = {'name': 'ASTM A572 Gr50', **MATERIAL_CATALOG.get('ASTM A572 Gr50', {})}
            mat_name = 'ASTM A572 Gr50'
            confidence = 0.6
            method = 'ultimate_fallback'
        ...
        logger.warning("ML material selection failed: %s, using ASTM A992 fallback", str(e))
        material = {'name': 'ASTM A992', **MATERIAL_CATALOG.get('ASTM A992', {})}
        material['_ml_selection'] = {
